...
export async function execute(id:string,store:Store,gateway:Gateway):Promise<Operation>{
 ...
 op.state='submitting';op.message='Checking current source availability…';
 try{const live=await gateway.read(op.action.target,op.action.intent.description);op.action=prepare(op.action.target,op.action.intent,live);}
 ...
 op.state='unknown';op.message='Result not confirmed. Do not repeat the request. Check the source portal or contact the jurisdiction.';await store.put(op);
...
export async function reconcile(op:Operation,store:Store,gateway:Gateway):Promise<Operation>{
 ...
 try{if(confirmed(op.action,await gateway.read(op.action.target))){op.state='succeeded';const source=op.action.target.jurisdiction===0?op.action.target.city:'MBP';op.message=(op.action.intent.kind==='schedule'?'Scheduled':'Cancelled')+' — confirmed in '+source+'.';await store.put(op);await store.release(lockKey(op),op.id);}}catch{/* Preserve uncertainty and the durable lock. */}
 return op;
}
