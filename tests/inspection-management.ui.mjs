...
try{
 for(const outcome of ['succeeded','failed','unknown']){
  ...
  await page.goto('http://127.0.0.1:4198');assert.equal(await page.getByRole('dialog').count(),0);await page.locator('tbody tr').filter({hasText:'Footing'}).getByRole('button',{name:'Schedule',exact:true}).click();assert.equal(submits,0);await page.getByRole('button',{name:'Close inspection management'}).click();assert.equal(await page.getByRole('dialog').count(),0);await page.locator('tbody tr').filter({hasText:'Footing'}).getByRole('button',{name:'Schedule',exact:true}).click();await page.getByLabel('Private management key').fill('mock-only-key');await page.getByRole('button',{name:'Unlock management'}).click();
